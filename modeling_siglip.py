...
class SiglipAttention(nn.Module):
    """ Multi-head attention layer from 'Attention is all you need' paper """
    ...
    def forward(self, hidden_states: torch.Tensor) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        ...
        # query_states : [Batch_Size, Num_Patches, Embed_Dim] -> [Batch_Size, Num_Heads, Num_Patches, Head_Dim]
        query_states = query_states.view(batch_size, seq_len, self.num_heads, self.head_dim).transpose(1, 2)
        # key_states : [Batch_Size, Num_Patches, Embed_Dim] -> [Batch_Size, Num_Heads, Num_Patches, Head_Dim]
        key_states = key_states.view(batch_size, seq_len, self.num_heads, self.head_dim).transpose(1, 2)
        # value_states : [Batch_Size, Num_Patches, Embed_Dim] -> [Batch_Size, Num_Heads, Num_Patches, Head_Dim]
        value_states = value_states.view(batch_size, seq_len, self.num_heads, self.head_dim).transpose(1, 2)

        # Calculate the attention using the formula Q * K^T / sqrt(d_k). attn weights : [Batch_Size, Num_Heads, Num_Patches, Num_Patches]
